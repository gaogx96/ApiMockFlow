...
export default function ApiTester() {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      {/* Tab Bar */}
      <div className="flex items-center border-b border-gray-100 bg-gray-50 shrink-0" style={{ height: 28 }}>
        <div ref={tabScrollRef} className="flex-1 flex items-center overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
          {tabs.map((t, i) => (
            <div
              key={t.id}
              onClick={() => setActiveIdx(i)}
              className={`flex items-center gap-1 px-2 h-7 text-xs cursor-pointer border-r border-gray-200 shrink-0 max-w-[120px] ${
                i === activeIdx ? 'bg-white text-primary-600 font-medium' : 'text-gray-500 hover:bg-gray-100'
              }`}
            >
      ...
      {/* Save dialog */}
      {showSaveDialog && (
        <div className="px-2 py-1.5 border-b border-gray-100 bg-blue-50 shrink-0 flex gap-1.5 items-center">
      ...
      {/* Import panel */}
      {showImport && (
        <div className="px-2 py-1.5 border-b border-gray-100 bg-gray-50 shrink-0">
          <textarea
          ...
          <div className="flex gap-1.5 mb-1.5">
            ...
            <button onClick={() => { setShowImport(false); setImportText(''); setImportedReqs([]); }}
              className="px-2.5 py-1 text-xs text-gray-500 border border-gray-200 rounded font-medium">取消</button>
          </div>
          {importedReqs.length > 0 && (
            <div className="bg-white rounded border border-gray-200 max-h-24 overflow-y-auto">
              ...
              {importedReqs.map((r, i) => (
                <div key={i} onClick={() => importOneToNewTab(r)}
                  className="flex items-center gap-1.5 px-2 py-1 cursor-pointer hover:bg-gray-50 text-xs border-b border-gray-50 last:border-0">
      ...
      {/* Sub Tabs */}
      <div className="flex border-b border-gray-100 px-2 bg-gray-50 shrink-0">
        {SUB_TABS.map(st => (
          <button key={st.key}
            ...
            {tab.response && (
              <div className="space-y-2">
                ...
                <details>
                  ...
                  <div className="mt-1 bg-gray-50 rounded p-1.5 text-xs max-h-20 overflow-y-auto" style={{ fontSize: 10 }}>
                ...
                <div>
                  ...
                  <pre className="bg-gray-50 rounded p-1.5 text-xs max-h-44 overflow-y-auto whitespace-pre-wrap break-all" style={{ fontSize: 10 }}>
        ...
        {activeSubTab === 'saved' && (
          <div className="divide-y divide-gray-50">
            {saved.length === 0 ? (
              ...
              saved.map((item) => (
                <div key={item.id}
                  className="px-2 py-1.5 cursor-pointer hover:bg-gray-50 transition-colors group flex items-center"
                  onClick={() => loadRequestToTab(item.request)}>
