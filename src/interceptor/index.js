// Main-world interceptor — 运行时层（fetch/XHR patch、消息桥、badge）。
// 匹配/改写引擎已收敛到 ../engine/engine（单一 TS 源）；本文件保持 verbatim 运行时逻辑，
// 仅消费引擎导出。构建：vite.interceptor.config.ts 以 lib/IIFE 打成自包含 dist/interceptor.js。
import {
  getMatchingRules,
  applyReq,
  applyResp,
  deleteHeaderCI,
  syncRules,
  setActive as engineSetActive,
  setGlobalEnabled as engineSetGlobalEnabled,
} from '../engine/engine';
...
(function () {
  ...
  // 保留在运行时层（verbatim）：它是浏览器侧工具、从来只有一份，不属于「引擎双实现」问题，
  // 经 applyReq 的 opts.crypto 注入引擎。
  ...
  // injectScript 抛错的白盒化上报：把报错暴露给测试人员（页面控制台 + 一条 INJECT_ERROR 日志），
  // 而非静默吞掉。作为 opts.onInjectError 注入引擎的 applyReq。
  function onInjectError(err, info) {
    var errMsg = 'injectScript 执行错误: ' + (err && err.message ? err.message : String(err));
    console.warn('[ApiMockFlow]', errMsg, '\n脚本内容:', (info.script || '').slice(0, 200));
    postLog({ id: logId(), timestamp: Date.now(), url: info.url, method: 'INJECT_ERROR', ruleIds: [], ruleNames: [], originalRequest: { headers: info.headers, body: trunc(info.body, 200) }, modifiedRequest: { url: info.url, headers: info.headers, body: trunc(info.body, 200) }, originalResponse: undefined, modifiedResponse: { status: 0, statusText: errMsg, headers: {}, body: '' }, cancelled: false, delayed: false, delayMs: 0 });
  }
  // 引擎 applyReq 的副作用注入：浏览器侧提供 crypto + 报错上报（测试/默认省略即纯逻辑）。
  var APII_REQ_OPTS = { crypto: API_CRYPTO, onInjectError: onInjectError };
  ...
  async function interceptedFetch(input, init) {
    ...
    var rm = applyReq(url, origH, origB, effectiveReqA, APII_REQ_OPTS);
  ...
  function patchXHR() {
    ...
    NATIVE_XHR.prototype.send = function (body) {
      ...
      var rm = applyReq(ou, orh, ob, effectiveReqA, APII_REQ_OPTS);
  ...
  function setActive(on) {
    if (on === ACTIVE) return;
    ACTIVE = on;
    engineSetActive(on); // 同步引擎门控（getMatchingRules 依赖 ACTIVE）
    if (on) {
  ...
  window.addEventListener('message', function (e) {
    ...
    if (e.data.type === 'APII_SYNC') {
      ...
      engineSetGlobalEnabled(e.data.globalEnabled); // 同步引擎门控
      var _rules = e.data.rules || [];
      ...
      for (var i = 0; i < _rules.length; i++) {
        if (_rules[i].match && _rules[i].match.url) _rules[i].match.url = _rules[i].match.url.trim();
      }
      syncRules(_rules, e.data.groups || []); // 置 RULES/GROUPS 并重建匹配索引
